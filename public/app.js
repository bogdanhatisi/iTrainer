...
let currentMinHipFlexion = 180;
let currentShinFlexion = 180;
let currentHeadTilt = 0;

let isForwardLeaning = false;
...
function draw() {
  ...
  if (poses.length > 0) {
    ...
    if ((isSquatting) & (nose.confidence > 0.75) & (leftEye.confidence > 0.75) && (rightEye.confidence > 0.75)) {
...
function validateDepthConstraints() {
  if (currentMinKneeFlexion > executionConstraints.depthSuperiorRange) {
    logs.innerHTML = '<p style="color: red;">Your depth is too high, you must go lower!</p>' + logs.innerHTML;
    ...
    logs.innerHTML =
      '<p style="color: green;">Perfect depth! Keep it going!</p>' + logs.innerHTML;
  }
}
...
function validateLeanConstraints() {
  if (
    ...
    logs.innerHTML = '<p style="color: red;">You lean too forward! Straighten your spine!</p>' + logs.innerHTML;
  } else if (
    currentMinLeanFlexion > executionConstraints.maximumForwardLeanRange
  ) {
    logs.innerHTML =
      '<p style="color: orange;">Your posture seems too straight! This may be affected by your depth.</p>' + logs.innerHTML;
  } else {
    logs.innerHTML =
...
function validateHeadConstraints() {
  if (currentHeadTilt == -1) {
    logs.innerHTML = '<p style="color: red;">Don\'t look down! Keep your head forward!</p>' + logs.innerHTML;
  } else if (currentHeadTilt == 1) {
    logs.innerHTML = '<p style="color: orange;">Your head tilt had a good positioning, but maybe looking forward instead of up would be more comfortable for you!</p>' + logs.innerHTML;
  } else {
    logs.innerHTML = '<p style="color: green;">Your head tilt had a good positioning! Nice!</p>' + logs.innerHTML;
  }
}
...
function validateTimeUnderTensionConstraints() {
  ...
  if (2 * eccentricElapsedTime > elapsedTime) {
    logs.innerHTML = '<p style="color: red;">You are going down too fast! Try to go down slower!</p>' + logs.innerHTML;
  } else {
    logs.innerHTML = '<p style="color: green;">Great balance between eccentric and concentric phase! Keep it going!</p>' + logs.innerHTML;
  }
}
